...
void Class_Robotic_arm::Init_Maxtrix()
{
	...
	DH_arm_motor[0].MAX_Angle=PI*0.9f;
	DH_arm_motor[0].MIN_Angle=-PI*0.9f;
	DH_arm_motor[0].Rotation=Reverse_Rotation;
	DH_arm_motor[0].Reduction_Ratio=3.0;

	DH_arm_motor[1].a=0.390f;
	DH_arm_motor[1].d=0.0f;
	DH_arm_motor[1].alpha=0;
	DH_arm_motor[1].Now_Angle=0.0f;
	DH_arm_motor[1].bias=0.2474f;
	DH_arm_motor[1].MAX_Angle=PI*0.8f;
	DH_arm_motor[1].MIN_Angle=0;
	DH_arm_motor[1].Rotation=Reverse_Rotation;
	DH_arm_motor[1].Reduction_Ratio=1.0;

	DH_arm_motor[2].a=0.360f;
	DH_arm_motor[2].d=0.0f;
	DH_arm_motor[2].alpha=0.0f;
	DH_arm_motor[2].Now_Angle=0.0f;
	DH_arm_motor[2].bias=0;
	DH_arm_motor[2].MAX_Angle=PI*0.1f;
	DH_arm_motor[2].MIN_Angle=-PI*0.8f;
	...
	DH_arm_motor[3].Reduction_Ratio=2.0;
 ...
 * @param 目标点坐标x,y,z
 */
int Class_Robotic_arm::Kinematics_Inverse_Preprocessing(float x, float y, float z)
{
 ...
 */
Matrix4x4 Class_Robotic_arm::Calculate_kinematics_Inverse(float x, float y, float z, int* err_flag, Pose_Orientation Attitude)
{
...
float Class_Robotic_arm::Cal_theta3(float K1,float K2,float a2,float a3,int* err_flag)
{
 ...
 */
void Class_Robotic_arm::Choose_Best_Angle_Group_Processing(Matrix4x4 Angle_Group,int* err_num,int err_group_flag[4])
{
	int err_angle_flag=0;
	for (int i=0;i<4;i++)
	{
		err_angle_flag=0;
		for (int j=0;j<4;j++)
 ...
 * @param Angle_Group:解算出的四组角度
 */
int Class_Robotic_arm::Choose_Best_Angle_Group(Matrix4x4 Angle_Group, float Last_Best_Group[4], int* err_flag)
{
	int flag=0;
	int err_group_flag[4]={0};		//0为有效
	int err_num[1]={0};
 ...
 * @param a[6]:方程的六个参数
 */
void Class_Robotic_arm::Joint_Space_Via_Path_Planning(int Ordinal_Num, float a[6])
{
 ...
 */
void Class_Robotic_arm::Joint_Space_Automatic_Para_Generation(int best_flag[])
{
	//起点特殊处理
	for (int j=0;j<4;j++)
	{
 ...
 * 返回值1表示规划成功，0为规划失败
 */
int Class_Robotic_arm::Joint_Space_Preprocessing()
{
	// Matrix4x4 Target_Angle_Matrix[acc_num];
	int err_flag[1]={0};		//每次在Calculate_kinematics_Inverse中初始化
 ...
 * 返回值1表示规划成功，0为规划失败
 */
int Class_Robotic_arm::Joint_Space_Dynamic_Tuning()
{
	int err_flag[1]={0};		//每次在Calculate_kinematics_Inverse中初始化
	float Last_Best_Group[4]={DH_arm_motor[0].Now_Angle,DH_arm_motor[1].Now_Angle,DH_arm_motor[2].Now_Angle,DH_arm_motor[3].Now_Angle};
	//先筛选出最佳角度方案
...
int main()
{
    ...
    for(int i=0;i<POS_MAX_NUM;i++)
    {
        printf("%.2f %.2f %.2f %.2f %.2f",RoboticArm.position[i][0],RoboticArm.position[i][1],RoboticArm.position[i][2],RoboticArm.position[i][3],RoboticArm.position[i][4]);
    }
    for(int i=0;i<POS_MAX_NUM;i++)
    {
        printf("%.2f %.2f %.2f %.2f",RoboticArm.Target_Angle_Matrix[i].Matrix[RoboticArm.best_flag[i]]);
