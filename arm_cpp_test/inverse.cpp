...
//范围为(-PI,PI]
Matrix4x4 Calculate_kinematics_Inverse(float x,float y,float z)
{
	float a1=0;// 关节2参数
	float a2=0.39;// 关节3参数
	float a3=0.36;// 关节4参数
	...
	K2_1=a4+z;
	float C=(K1_1*K1_1+K2_1*K2_1-a2*a2-a3*a3)/(2*a2*a3);
	result.Matrix[0][2]=acos(C);
	result.Matrix[1][2]=-result.Matrix[0][2];
	//解2_1 解2_2
	K1_2=x/cos(result.Matrix[2][0])-a1;
	K2_2=a4+z;
	result.Matrix[2][2]=acos((K1_2*K1_2+K2_2*K2_2-a2*a2-a3*a3)/(2*a2*a3));
...
float Cal_theta2(float theta3,float K1,float K2,float a2,float a3)
{
	...
	else if (c2>1e-3)					//t在一四象限
		theta2_11=asin(s2);
	else if (c2<-1e-3&&s2>1e-3)			//t在第二象限
		theta2_11=acos(c2);
	else if (c2<-1e-3&&s2<-1e-3)			//t在第三象限
		theta2_11=-acos(c2);
	return theta2_11;
}
// 示例使用
int main() {
    ...
    Matrix4x4 result=Calculate_kinematics_Inverse(-0.14248,-0.14248,0.55381);
    // 打印矩阵验证（此时应为单位矩阵）
    for (int i=0; i<4; i++) {
        for (int j=0; j<4; j++) {
