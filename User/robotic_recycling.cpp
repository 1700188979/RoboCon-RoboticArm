...
{
	recycling_motor_elevator.Motor.Init(&hfdcan2,Motor_DJI_ID_0x201,Motor_DJI_Control_Method_ANGLE);
	recycling_motor_elevator.Motor.PID_Omega.Init(2.5f, 0.00f, 0.0f, 0.0f, 10.0f * PI, 16.0f * PI, 0.001f,0.01);
	recycling_motor_elevator.Motor.PID_Angle.Init(5.0f, 0.000f, 0.001f, 0.0f, 10.0f * PI, 16.0f * PI, 0.001f,0.01);

	recycling_motor_transport.Motor.Init(&hfdcan2,Motor_DJI_ID_0x202,Motor_DJI_Control_Method_ANGLE);
	recycling_motor_transport.Motor.PID_Omega.Init(2.5f, 0.00f, 0.0f, 0.0f, 10.0f * PI, 16.0f * PI, 0.001f,0.01);
	recycling_motor_transport.Motor.PID_Angle.Init(5.0f, 0.000f, 0.001f, 0.0f, 10.0f * PI, 16.0f * PI, 0.001f,0.01);
}

void Class_Robotic_Recycling::Set_Target_point(Robotic_Recycling_Motor_Type Type, float distance, float T,
 ...
 * @brief 清理目标点
 */
void Class_Robotic_Recycling::Clear_Path_Planning(Robotic_Recycling_Motor_Type Type)
{
	//清除a_quintic
	for (int j=0;j<6;j++)
	{
 ...
 * @param a
 */
void Class_Robotic_Recycling::Joint_Space_Quintic_Cal_Via_Para(float T,float theta0,float thetaf,Enum_Order_OF_Robotic_Recycling_Path_Planning_Curve Target_Order_Num,float a[6])
{
	if (T <= 0) {
		return ;// 检查时间间隔有效性
	}
 ...
 */
void Class_Robotic_Recycling::Joint_Space_Via_Path_Planning(Robotic_Recycling_Motor_Type Type)
{
...
void Class_Robotic_Recycling::Robotic_Recycling_TIM_10ms_PeriodElapsedCallback()
{
	...
	recycling_motor_transport.Motor.Set_Target_Omega(recycling_motor_elevator.Next_Omega);
	recycling_motor_transport.Motor.Set_Target_Angle(recycling_motor_elevator.Next_Angle);
	recycling_motor_transport.Motor.TIM_Calculate_PeriodElapsedCallback();
