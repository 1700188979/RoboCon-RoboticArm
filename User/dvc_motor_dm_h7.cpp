/**
 * @file dvc_motor_dm_h7.cpp
 * @brief 适配达妙H7开发板的达妙电机配置与操作
 *
 */
...
// 清除电机错误信息, 传统模式有效
uint8_t DM_Motor_FDCAN_Message_Clear_Error[8] = {0xff,
                                               0xff,
                                               0xff,
                                               0xff,
                                               0xff,
                                               ...
                                               0xfb};
// 使能电机, 传统模式有效
uint8_t DM_Motor_FDCAN_Message_Enter[8] = {0xff,
                                         0xff,
                                         0xff,
                                         0xff,
                                         0xff,
                                         ...
                                         0xfc};
// 失能电机, 传统模式有效
uint8_t DM_Motor_FDCAN_Message_Exit[8] = {0xff,
                                        0xff,
                                        0xff,
                                        0xff,
                                        0xff,
                                        ...
                                        0xfd};
// 保存当前电机位置为零点, 传统模式有效
uint8_t DM_Motor_FDCAN_Message_Save_Zero[8] = {0xff,
                                             0xff,
                                             0xff,
                                             0xff,
                                             0xff,
 ...
 */
uint8_t *allocate_tx_data(FDCAN_HandleTypeDef *hfdcan, Enum_Motor_DM_Motor_ID_1_To_4 __CAN_Rx_ID_1_To_4)
{
    uint8_t *tmp_tx_data_ptr = nullptr;

    // 对比传入句柄与外部定义的句柄指针 (drv_fdcan_h7.h 中 extern hfdcan1/hfdcan2/hfdcan3)
    if (hfdcan == &hfdcan1)
    {
    ...
    else if (hfdcan == &hfdcan3)
    {
        ...
        {
        case (Motor_DM_ID_0x301):
            tmp_tx_data_ptr = &(FDCAN3_0x3fe_Tx_Data[0]);
            break;
        case (Motor_DM_ID_0x302):
            tmp_tx_data_ptr = &(FDCAN3_0x3fe_Tx_Data[2]);
            break;
        case (Motor_DM_ID_0x303):
            tmp_tx_data_ptr = &(FDCAN3_0x3fe_Tx_Data[4]);
            break;
        case (Motor_DM_ID_0x304):
            tmp_tx_data_ptr = &(FDCAN3_0x3fe_Tx_Data[6]);
            break;
        case (Motor_DM_ID_0x305):
            tmp_tx_data_ptr = &(FDCAN3_0x4fe_Tx_Data[0]);
            break;
        case (Motor_DM_ID_0x306):
            tmp_tx_data_ptr = &(FDCAN3_0x4fe_Tx_Data[2]);
            break;
        case (Motor_DM_ID_0x307):
            tmp_tx_data_ptr = &(FDCAN3_0x4fe_Tx_Data[4]);
            break;
        case (Motor_DM_ID_0x308):
            tmp_tx_data_ptr = &(FDCAN3_0x4fe_Tx_Data[6]);
            break;
        default:
            tmp_tx_data_ptr = nullptr;
            break;
...
/**
 * @brief 电机初始化（Normal 模式）
 *
 * @param hfdcan 绑定的 FDCAN 总线句柄
 * @param __CAN_Rx_ID 收数据绑定的 CAN ID (上位机 Master ID)
 * @param __CAN_Tx_ID 发数据绑定的 CAN ID (上位机 CAN_ID + 模式偏移)
 * @param __Motor_DM_Control_Method 控制方式
 * @param __Angle_Max 最大角度
 * @param __Omega_Max 最大速度
 * @param __Torque_Max 最大扭矩
 */
void Class_Motor_DM_Normal::Init(FDCAN_HandleTypeDef *hfdcan, uint8_t __FDCAN_Rx_ID, uint8_t __FDCAN_Tx_ID, Enum_Motor_DM_Control_Method __Motor_DM_Control_Method, float __Angle_Max, float __Omega_Max, float __Torque_Max, float __Current_Max)
{
    // 选择对应的管理对象指针
 ...
 * @param Rx_Data 接收的数据
 */
void Class_Motor_DM_Normal::FDCAN_RxCpltCallback(uint8_t *Rx_Data)
{
    // 滑动窗口, 判断电机是否在线
    Flag += 1;
 ...
 *
 */
void Class_Motor_DM_Normal::FDCAN_Send_Clear_Error()
{
    FDCAN_Send_Data(FDCAN_Manage_Object->FDCAN_Handler, FDCAN_Tx_ID, DM_Motor_FDCAN_Message_Clear_Error, 8, FDCAN_STANDARD_ID, FDCAN_DATA_TYPE);
}
 ...
 *
 */
void Class_Motor_DM_Normal::FDCAN_Send_Enter()
{
    FDCAN_Send_Data(FDCAN_Manage_Object->FDCAN_Handler, FDCAN_Tx_ID, DM_Motor_FDCAN_Message_Enter, 8, FDCAN_STANDARD_ID, FDCAN_DATA_TYPE);
}
 ...
 *
 */
void Class_Motor_DM_Normal::FDCAN_Send_Exit()
{
    FDCAN_Send_Data(FDCAN_Manage_Object->FDCAN_Handler, FDCAN_Tx_ID, DM_Motor_FDCAN_Message_Exit, 8, FDCAN_STANDARD_ID, FDCAN_DATA_TYPE);
}
 ...
 *
 */
void Class_Motor_DM_Normal::FDCAN_Send_Save_Zero()
{
    FDCAN_Send_Data(FDCAN_Manage_Object->FDCAN_Handler, FDCAN_Tx_ID, DM_Motor_FDCAN_Message_Save_Zero, 8, FDCAN_STANDARD_ID, FDCAN_DATA_TYPE);
}
...
void Class_Motor_DM_Normal::Data_Process()
{
    ...
    Struct_Motor_DM_FDCAN_Rx_Data_Normal *tmp_buffer = (Struct_Motor_DM_FDCAN_Rx_Data_Normal *)FDCAN_Manage_Object->Rx_Buffer.Data;

    // 电机ID不匹配, 则不进行处理
    if(tmp_buffer->FDCAN_ID != (FDCAN_Tx_ID & 0x0f))
    {
        return;
    }
...
void Class_Motor_DM_Normal::Output()
{
    ...
    case (Motor_DM_Control_Method_NORMAL_MIT):
    {
        Struct_Motor_DM_FDCAN_Tx_Data_Normal_MIT *tmp_buffer = (Struct_Motor_DM_FDCAN_Tx_Data_Normal_MIT *)Tx_Data;
        ...
        FDCAN_Send_Data(FDCAN_Manage_Object->FDCAN_Handler, FDCAN_Tx_ID, Tx_Data, 8, FDCAN_STDID, FDCAN_DATA_TYPE);

        break;
    }
    case (Motor_DM_Control_Method_NORMAL_ANGLE_OMEGA):
    {
        Struct_Motor_DM_FDCAN_Tx_Data_Normal_Angle_Omega *tmp_buffer = (Struct_Motor_DM_FDCAN_Tx_Data_Normal_Angle_Omega *)Tx_Data;
    ...
    case (Motor_DM_Control_Method_NORMAL_OMEGA):
    {
        Struct_Motor_DM_FDCAN_Tx_Data_Normal_Omega *tmp_buffer = (Struct_Motor_DM_FDCAN_Tx_Data_Normal_Omega *)Tx_Data;

        tmp_buffer->Control_Omega = Control_Omega;

        FDCAN_Send_Data(FDCAN_Manage_Object->FDCAN_Handler, FDCAN_Tx_ID, Tx_Data, 4, FDCAN_STDID, FDCAN_DATA_TYPE);

        break;
    }
    case (Motor_DM_Control_Method_NORMAL_EMIT):
    {
        Struct_Motor_DM_FDCAN_Tx_Data_Normal_EMIT *tmp_buffer = (Struct_Motor_DM_FDCAN_Tx_Data_Normal_EMIT *)Tx_Data;
 ...
 */
void Class_Motor_DM_1_To_4::Init(FDCAN_HandleTypeDef *hfdcan, Enum_Motor_DM_Motor_ID_1_To_4 __FDCAN_Rx_ID, Enum_Motor_DM_Control_Method __Motor_DM_Control_Method, int32_t __Encoder_Offset, float __Current_Max)
{
    // 选择对应的管理对象指针
    if (hfdcan->Instance == FDCAN1)
    {
 ...
 * @param Rx_Data 接收的数据
 */
void Class_Motor_DM_1_To_4::FDCAN_RxCpltCallback(uint8_t *Rx_Data)
{
    // 滑动窗口, 判断电机是否在线
    Flag += 1;
...
void Class_Motor_DM_1_To_4::Data_Process()
{
    ...
    Struct_Motor_DM_FDCAN_Rx_Data_1_To_4 *tmp_buffer = (Struct_Motor_DM_FDCAN_Rx_Data_1_To_4 *) FDCAN_Manage_Object->Rx_Buffer.Data;
