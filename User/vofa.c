 ...
 * @retval *
 */
void Vofa_Timeout(void)  //在中断里调用以下函数，和数据发送放在一起。目的：防止错误数据干扰后，接收不到正确的数据。
{
...
void Vofa_Transmit(UART_HandleTypeDef *huart, uint8_t num)
{
    ...
    HAL_UART_Transmit(huart, (uint8_t *)tempData, len,0xff);
     // HAL_UART_Transmit_IT(huart, (uint8_t *)tempData, len);
}
/**
 * @brief  VOFA帧处理函数
 *         通过此函数处理接收到的VOFA帧
...
//对vofa发送来的数据（例如FE EF 01 %% ED DE）(小端浮点数组)
//ED DE是尾帧
//01是身份帧，表示1号滑块（其中的第一位表示命令类型（0为滑块，1为按钮，2为按键，3为摇杆），第二位1表示该命令位号）
//%%为发过来的数据帧，表示滑块当前的数据
//FE EF为发过来的首帧


void Vofa_FRAME_Handler(VOFA_RxTypedef Vofa_RX)
{
    ...
    {
        FirstNum = (Vofa_RX.Buff[2] & 0xf0) >> 4;//命令类型，0为滑块，1为按钮，2为按键，3为摇杆
        LastNum = Vofa_RX.Buff[2] & 0x0f;//命令位号，是几就是几号命令
        switch (FirstNum)
        {
        case 0:
...
void Vofa_Slider_Handler(uint8_t Num)
{
    ...
    for (i = 0; i < 4; i++)
    {
        DataConvert.DataChar[i] = Vofa_RX.Buff[3 + i];//DataConvert.DataChar存储发来的四字节浮点数，Vofa_RX.Buff【3】~Vofa_RX.Buff【6】中得到
    }
    switch (Num)//匹配命令位号
    {
#ifdef UseVOFASlider1
    case 1:
        Vofa_Slider1 = DataConvert.DataFloat;//Vofa传来的滑块数据储存入Vofa_Slider1中
        break;
#endif
