 ...
 */
void Class_PID::Init(float __K_P, float __K_I, float __K_D, float __K_F, float __I_Out_Max, float __Out_Max, float __D_T,  float __Dead_Zone, float __I_Option, float __I_Separate_Threshold, float __I_Variable_Speed_A, float __I_Variable_Speed_B,  Enum_PID_D_First __D_First)
{
    K_P = __K_P;
    K_I = __K_I;
    K_D = __K_D;
...
void Class_PID::TIM_Calculate_PeriodElapsedCallback()
{
    ...
    {
        // 没有积分分离
        if(I_Option == 0.0f) Integral_Error += speed_ratio * D_T * error;
        else{
